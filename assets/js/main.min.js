...
return this},b.lib.init.prototype=b.lib,b}),window.addEventListener("resize",calcHiderTransform),window.addEventListener("keydown",keyboardShortcuts),window.addEventListener("click",checkIfPopoutIsOpen),window.addEventListener("blur",checkIfPopoutIsOpen);let iframe=document.querySelector("#theme-preview");iframe.addEventListener("load",updateIframeContent);async function updateIframeContent({build:a,directory:b,view:c}={}){let d=document.querySelector("#theme-preview");a&&(d.dataset.build=a,d.contentWindow.document.querySelector("#discord-css").href=`${a}/default.css`,console.log(d.contentWindow.document.querySelector("#discord-css").href)),b&&(d.dataset.directory=b),c&&(d.dataset.view=c);let e=d.dataset.build,f=d.dataset.directory,g=d.dataset.view,h=d.dataset.compact;document.documentElement.classList.add("loading-iframe"),await sleep(500),fetch(`${e}/${f}/${h?"compact/":""}${g}.html`).then(a=>200===a.status?a.text():Promise.reject()).then(async a=>{// Convert the HTML string into a document object
let b=new DOMParser;var c=b.parseFromString(a,"text/html");let d=document.querySelector("#theme-preview");d.contentWindow.document.body=c.body,discordDarkSidebar(),await sleep(500),document.documentElement.classList.remove("loading-iframe","iframe-didnt-load")}).catch(async()=>{// There was an error
